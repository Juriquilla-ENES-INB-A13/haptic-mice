...
{
  appendTextToFile(flname, "repeat,freq,door_time,poke_time,touched_poke,inside_time,status");
}
...
void writeParamsToFile(String flname)
{
  ...
  String params = new String(
    ...
    " open_door_list:"+ fld_door_time.getValueI()+
...
int randomizeDoorTime()
{
  int tmpDoorTime;
  String doorTimeList = fld_door_time.getText();
  String[] splitedDoorTime = doorTimeList.split(",");
  println("RUN:"+splitedDoorTime.length+" door delays to randomize!");
  tmpDoorTime = Integer.parseInt(splitedDoorTime[int(random(splitedDoorTime.length))]);
  println("RUN: Randomized door time:"+tmpDoorTime);
  return tmpDoorTime;
}
...
void startExperiment() {
  ...
  writeTableHeader(filename);  
    while((numIteration<=repeats)&&runExperiment){
      ...
      addWindowInfo();
      appendTextToFile(filename,numIteration+","+freq+","+door_time+","+pokeTime+","+whichPoke+","+insideTime+","+status);
